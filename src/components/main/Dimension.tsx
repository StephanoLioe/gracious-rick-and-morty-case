...
interface IDimensionListProps {
  ...
  setSelectedEpisode: React.Dispatch<React.SetStateAction<ILocation | null>>;
  selectedEpisode: ILocation | null;
}
...
const EpisodeList: React.FC<IDimensionListProps> = ({
  setCharacterIds,
  setSelectedEpisode,
  selectedEpisode = null,
}) => {
  const [page, setPage] = useState(1);
  const { data, status } = useQuery<IQueryResult>(["location", page], () =>
        ...
        data.results.map((location: ILocation) => (
          <div
            className={`context-list ${
              location.id === selectedEpisode?.id ? "active" : ""
            }`}
            key={location.id}
            onClick={() => {
              handleCharacterIds(location.residents);
...
export const Dimension: React.FC = () => {
  ...
  const [selectedEpisode, setSelectedEpisode] = useState<ILocation | null>(
    null
  );
